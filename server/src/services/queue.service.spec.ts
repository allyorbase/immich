...
describe(QueueService.name, () => {
  ...
  describe('onConfigUpdate', () => {
    it('should update concurrency', () => {
      ...
      expect(mocks.job.setConcurrency).toHaveBeenNthCalledWith(1, QueueName.AssetStacking, 1);
  ...
  describe('getAllJobStatus', () => {
    it('should get all job statuses', async () => {
      ...
      await expect(sut.getAllLegacy(factory.auth())).resolves.toEqual({
        [QueueName.AssetStacking]: expected,
  ...
  describe('handleCommand', () => {
    ...
    it('should handle a start asset stacking command', async () => {
      mocks.job.isActive.mockResolvedValue(false);
      mocks.job.getJobCounts.mockResolvedValue(factory.queueStatistics());

      await sut.runCommandLegacy(QueueName.AssetStacking, { command: QueueCommand.Start, force: false });

      expect(mocks.job.queue).toHaveBeenCalledWith({ name: JobName.AssetStacking });
    });
